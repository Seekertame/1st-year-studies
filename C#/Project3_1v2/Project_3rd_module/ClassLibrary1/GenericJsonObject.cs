...
namespace ClassLibrary1
{
    ...
    public class GenericJsonObject : IJSONObject
    {
        ...
        {
            return _data.TryGetValue(key, out object? collectionObj) && collectionObj is List<object> collection ? collection : null;
        }
        ...
        public void FilterCollection(string collectionKey, string fieldName, List<string> allowedValues)
        {
            ...
            if (collectionObj is not List<object> collection)
            {
        ...
        public void SortCollection(string collectionKey, string sortField, bool ascending)
        {
            ...
            // Собираем уникальные ключи из всех объектов коллекции (если элемент является словарём)
            HashSet<string> uniqueKeys = new(StringComparer.InvariantCultureIgnoreCase);
            foreach (object item in collection)
            {
                if (item is Dictionary<string, object> dict)
                {
            ...
            // Если поле для сортировки отсутствует во всех объектах, сообщаем об ошибке и выходим
            if (!uniqueKeys.Contains(sortField))
            {
                throw new Exception($"Поле сортировки '{sortField}' отсутствует во всех элементах коллекции.");
            }
            ...
            int CompareItems(object o1, object o2)
            {
                ...
                if (o1 is Dictionary<string, object> d1 && o2 is Dictionary<string, object> d2)
                {
                    ...
                    _ = d1.TryGetValue(sortField, out object? v1);
                    _ = d2.TryGetValue(sortField, out object? v2);
            ...
            // Сортируем коллекцию с использованием лямбда-выражения.
        ...
        private string SerializeValuePretty(object value, int indentLevel)
        {
            ...
            string indentChild = new(' ', (indentLevel + 1) * 4);
            ...
            if (value is Dictionary<string, object?> dict)
            {
                ...
                foreach (KeyValuePair<string, object?> kvp in dict)
                {
                    ...
                    _ = sb.Append(indentChild);
                    _ = sb.Append($"\"{EscapeString(kvp.Key)}\": ");
                    _ = sb.Append(SerializeValuePretty(kvp.Value!, indentLevel + 1));
                }
                _ = sb.AppendLine();
                _ = sb.Append(indent);
                _ = sb.Append('}');
            ...
            if (value is List<object> list)
            {
                ...
                foreach (object item in list)
                {
                    ...
                    _ = sb.Append(indentChild);
                    _ = sb.Append(SerializeValuePretty(item, indentLevel + 1));
                }
                _ = sb.AppendLine();
                _ = sb.Append(indent);
                _ = sb.Append(']');
